...
import type { Route } from "./+types/employees.page";
...
export default function AdminEmployeesPage() {
  ...
  const employees = (data?.users ?? []).filter((user) =>
    normalizeRoles(user.role).every((role) => role !== "admin")
  );
  ...
  const pageCard = isAdmin ? (
    <Card className="border-border">
      <CardHeader>
        ...
        <CardDescription>View employee access and roles.</CardDescription>
      </CardHeader>
      <CardContent>
        <div className="pt-4">
          {isLoading ? (
            ...
            <EmployeesList users={employees} total={employees.length} />
          )}
        </div>
      </CardContent>
    </Card>
    ...
    <Card className="border-destructive/30">
      <CardHeader>
        ...
        <CardDescription>You need admin permissions to manage employees.</CardDescription>
      </CardHeader>
    </Card>
  );
...
function EmployeesList({ users, total }: { users: AdminUser[]; total: number }) {
  if (!users.length) {
    return (
  ...
  return (
    <div className="space-y-2">
      <p className="text-xs text-muted-foreground">Total employees: {total}</p>
...
function EmployeesError({
  ...
  return (
    <div className="flex flex-col items-start gap-3 rounded-lg border border-destructive/30 bg-destructive/10 p-4 text-destructive">
      <div>
        <p className="text-sm font-medium">We couldn&apos;t load employees.</p>
...
function normalizeRoles(role: AdminUser["role"]) {
  if (!role) {
    return [];
  }
